    ...
    local function databox(typename, itemname, pages_done, pages_total, time_left_text, pages_done_display, pages_total_display, options)
        ...
        if not options.hide_title then
            table.insert(elements, TextWidget:new{
                text = typename,
                face = Font:getFace(db_font_italics, db_font_size_tiny),
                bold = true,
...
--battery
    ...
    local chapterbox = content_mode ~= CONTENT_MODE_HIGHLIGHT_PROGRESS and databox("Chapter", chapter_title, chapter_done, chapter_total, chapter_time_left, nil, nil, {
